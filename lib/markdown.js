...
exports.convertBatchToMarkdown = convertBatchToMarkdown;
exports.formatSourceList = formatSourceList;
exports.formatGroupList = formatGroupList;
 ...
 * - 补充内容
 */
function convertToMarkdown(item, pushTitle) {
    const lines = [];
    // 标题（粗体显示，消除与正文的重复）
 ...
 */
function convertBatchToMarkdown(items, pushTitle) {
    if (items.length === 0)
        return '';
 ...
 * 获取推文类型图标
 */
function getTypeIcon(tweetType) {
    const icons = {
        reply: '💬 ',
        retweet: '🔁 ',
        original: '',
 ...
 */
function removeTitleDuplicate(title, content) {
    if (!content)
        return '';
    if (!title)
        return content.trim();
